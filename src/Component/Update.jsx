...
const data = useParams();
  ...
  const fetchData = async () =>{
      try{
        ...
        const get = await fetch(`http://localhost:3002/bank/contact/${data.id}`)
 ...
 useEffect(()=>{
   const fetchData = async () =>{
     try{
       ...
       const get = await fetch(`http://localhost:3002/bank/${data.id}`);
      const getValue = await get.json();
...
const handleSubmit = async (event) => {
    ...
    try{
      axios.put(`http://localhost:3002/bank/post/${details.name}`,{
         balance:cost
      })
      axios.put(`http://localhost:3002/bank/post/${change.receiver}`,{
         balance:costValue
      })
      .then(res=>{
        console.log(res);
